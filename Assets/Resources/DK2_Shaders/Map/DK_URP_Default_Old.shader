Shader "DK2_URP_Shader/Map/BG_Default_Old"
{
	Properties
	{
		...
		[Toggle(Glow)] _Glow("Glow on", float) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			sampler2D _MainTex,_MaskTex, _GlowTex, _BumpMap, _BlendTex,_MoveTex,_ReflectionTexUp;
			samplerCUBE		_Cube;
			
			float4 _MainTex_ST, _MoveTex_ST, _BumpMap_ST, _GlowTex_ST, _BlendTex_ST,_MaskTex_ST,
			...
			struct tOUTPUT
			{
				...
				#ifdef Leaf
				float3 InnerAo 		: TEXCOORD8;
				float3 UnmapedNorml : TEXCOORD9;
			...
			tOUTPUT vert_Main(Vertexinput v)
			{
				...
				#ifdef VertexAnimation  
					tOut.vertex.xyz += float3(VertexPositionBasedRandomMovement(tOut.worldPos).xyz * v.vcolor.g *  tOut.v2UV.y) ;			
				#endif
				...
				#ifdef Leaf
				tOut.UnmapedNorml = tOut.worldPos - TransformObjectToWorld(float3(0,0,0)) ;
				tOut.InnerAo = normalize( v.vertex.xyz -_WorldposY.xyz) ;
				#endif
			...
			float4 frag_Main(tOUTPUT tIn) : SV_Target
			{
				...
				float4	MaskTexColor = tex2D(_MaskTex, TRANSFORM_TEX(tIn.v2UV, _MainTex)) ;
				float3 normalDirection = normalize(tIn.Normal);
				float3 ambientColor = SampleSH(normalDirection) ; 
				// Replace Color
				...
				// Normal Map
					...
					float3  Normal = UnpackNormal(tex2D(_BumpMap,  TRANSFORM_TEX (tIn.v2UV,_MainTex)));
				...
				float3 NdotL = _NoAmbient ? 1 : max(0, dot(lightDir, normalDirection));
				NdotL *= Mainlightcol.rgb*  mainLight.distanceAttenuation *(mainLight.shadowAttenuation ) ;
				NdotL +=  ambientColor ;
				...
				//addLight
					...
					for (uint lightIndex = 0u; lightIndex < pixelLightCount; ++lightIndex)
					{
						...
						NdotL +=  addLightCol * addLight.color.rgb * Shadowdist *  distance;
			  ...
			  #ifdef Reflection
			  {
					...
					#ifdef RealTimeReflection
				{
					...
					f4TexColor.rgb = lerp(f4TexColor.rgb,Realtimeref,_Cubelerp);
				}
				#else
				{
					...
					f4TexColor.rgb = lerp( f4TexColor.rgb  ,  refcolor *_CubeIntencity * _CubeColor.rgb, _Cubelerp * MaskTexColor.g);
				}
				#endif
				...
				// 메인 컬러
				float3 finalDiff = NdotL * f4TexColor.rgb   ;
			...
			finalDiff.rgb *=  _Color.rgb *_Intencity;
			...
			#ifdef Specular 
			{
				...
				float3 halfDir = SafeNormalize(ViewDir + lightDir * mainLight.distanceAttenuation);

				float3 fianlhalfDir = _CustomLight ? normalize (ViewDir + customlight) : halfDir;  
		
			///////// Gloss:
				float gloss = _Smoothness   ;
				float specPow = exp2(gloss * 10.0 + 1.0 *  MaskTexColor.r) ;
			//Specular0
				float3 SpeDir =max(0,dot(fianlhalfDir, normalDirection));
			...
			//float SpeDir1 = saturate(dot(halfDir, Normal));
				float3 SpecCol = pow(SpeDir ,  specPow) *MaskTexColor.g *  _SpecColor.rgb * Mainlightcol;
				finalDiff.rgb +=  SpecCol;
			...
			// Glow Texture
				#ifdef Glow
					float4	shiningcolor = _GlowTexture ? (pow(abs(tex2D(_GlowTex, TRANSFORM_TEX(tIn.v2UV2.xy  + _Speed.xy * _Time.xx, _GlowTex)).r +tex2D(_GlowTex, TRANSFORM_TEX( 1 - tIn.v2UV2.xy   + _Speed.zw * _Time.xx, _GlowTex)).r * 0.5),_GlowPow)) :1;
					float3 GlowColor = 	_GlowTexture ? lerp( finalDiff.rgb  , _GlowColorAni.rgb , max(0,shiningcolor.rgb)) : _GlowColor.rgb;
					finalDiff = (_Glow ? lerp( finalDiff , f4TexColor  * GlowColor.rgb , MaskTexColor.b * shiningcolor) : finalDiff);
				#endif
