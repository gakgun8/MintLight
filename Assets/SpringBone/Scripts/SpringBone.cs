...
namespace DynamicBone
{
	public class SpringBone : MonoBehaviour
	{
		...
		float ScaledRadius => Mathf.Abs(radius * transform.lossyScale.x);
		...
		public void UpdateSpring ()
		{
			...
			//drag
			force += (prevTipPos - currTipPos) * dragForce / (sqrDt * Mathf.Abs(transform.lossyScale.x));

			force += springForce / sqrDt;

			force *= Mathf.Abs(transform.lossyScale.x);
		...
		public SpringBone Copy(GameObject target)
		{
			var sb = target.AddComponent<SpringBone>();
			sb.child = target.transform.Find(child.GetPath(transform));
			sb.boneAxis = boneAxis;
			sb.radius = radius;
			sb.isUseEachBoneForceSettings = isUseEachBoneForceSettings;
			sb.stiffnessForce = stiffnessForce;
			sb.dragForce = dragForce;
			sb.springForce = springForce;
			sb.debug = debug;
			sb.threshold = threshold;
			sb.colliders = new SpringCollider[colliders.Length];
			sb.Awake();
			sb.Start();
			return sb;
		}
